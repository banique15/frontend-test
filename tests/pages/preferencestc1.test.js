  ...
  default: {
    ...
    searchMembers: {
      search: vi.fn(() => Promise.resolve({
        exact_matches: {
          total_items: 1,
          members: [{ email_address: 'notsubscribed@example.com', status: 'unsubscribed' }],
        },
      })),
    },
  },
...
describe('User Email Handling', () => {
  it('fetches existing Mailchimp member details', async () => {
    ...
    const result = await mailchimpFunctions.search('notsubscribed@example.com');
    ...
    expect(searchSpy).toHaveBeenCalledWith('notsubscribed@example.com');
    // Clean up the spy
