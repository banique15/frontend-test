  ...
  default: {
    ...
    searchMembers: {
      search: vi.fn(() => Promise.resolve({
        exact_matches: {
          total_items: 1,
          members: [{ email_address: 'subscribed@example.com', status: 'subscribed' }],
        },
      })),
    },
  },
...
describe('User Email Handling', () => {
  it('fetches existing Mailchimp member details', async () => {
    ...
    const result = await mailchimpFunctions.search('subscribed@example.com');
    ...
    expect(searchSpy).toHaveBeenCalledWith('subscribed@example.com');
    // Clean up the spy
